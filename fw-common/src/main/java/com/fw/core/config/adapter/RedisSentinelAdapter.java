...
import com.mbv.core.config.RedisSentinelConfig;
import com.mbv.core.util.CommonUtil;
...
@Component
@Log4j2
@ConditionalOnProperty(name = "redis.config.type", havingValue = "sentinel")
public class RedisSentinelAdapter implements RedisAdapter{ //implements DisposableBean
    ...
    private JedisSentinelPool getRedisPool() {
        if (jedisPool == null) {
            //(String masterName, Set<String> sentinels, GenericObjectPoolConfig<Jedis> poolConfig, int timeout, String password, int database)
            jedisPool = new JedisSentinelPool(redisSentinelConfig.getMasterName(), sentinelUri, jedisPoolConfig, redisSentinelConfig.getTimeout(), redisSentinelConfig.getMasterPassword(), redisSentinelConfig.getDatabase());
        }
        return jedisPool;
    }
    ...
    public long sendJobToWaitQueue(String queue, String job) {
        ...
        try {
            ...
            log.error("sendJobToWaitQueue redis error : " + e.getMessage());
            return -1;
        } finally {
            returnToPool(jedis);
        }
